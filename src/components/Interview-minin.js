...
import {
  interviewMininData,
  getSec,
} from './interview-minin-data';



function InterviewMinin () {

  useEffect(() => {
    console.count("useEffect Inteview-minin Component")
  }, [])
  ...
  return (
    <>
      ...
      <main className='interview__iframes'>
        {interviewMininData.map(item => {
          return (
            <section className="interview__frame-container" key={item.frameName}>
              {videoFrame(item)}
            </section>
          )
          })
